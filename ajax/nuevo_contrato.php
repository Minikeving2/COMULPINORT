...
mysqli_query($con,"SET NAMES 'utf8'");
	...
	if ($sql){	
    	...
    	if (copy($ruta, $destino)) {
			...
			echo json_encode('<script>alert("Contrato registrado pero el tipo de archivo no es valido") </script>');
			exit;
		}
		...
		echo json_encode("<script>alert('Contrato registrado correctamente')</script>");
 	} else {
		echo json_encode("<script>alert('Lo sentimos, no se registro el contrato')</script>");
 	}
